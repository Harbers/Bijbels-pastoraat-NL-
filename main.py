...
# Stel logging in (hier wordt debug-level gebruikt)
...
# Google Custom Search API instellingen (optioneel)
...
def extract_verse(text: str, psalm: int, vers: int) -> str:
    # Probeer meerdere patronen losser af te stemmen:
    patterns = [
    ...
    lines = re.split(r'\n+', text.strip())
    logger.debug(f"Extracted {len(lines)} lijnen uit psalm {psalm}")
    if 1 <= vers <= len(lines):
...
def get_unique_psalm_url(psalm: int, vers: int) -> str:
    ...
    if link and link.get("href"):
        ...
        return unique_url
    logger.debug("Geen uniek URL gevonden, gebruik standaard URL")
    return base_url
...
def get_psalm_text_psalmboek(psalm: int, vers: int) -> str:
    ...
    logger.debug(f"Volledige tekst verkregen (eerste 200 karakters): {full_text[:200]}...")
